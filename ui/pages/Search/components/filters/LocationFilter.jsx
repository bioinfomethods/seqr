...
const { additionalFormFields, fieldDisplay, isEditable, validate, ...LOCUS_LIST_ITEMS_BASE_FIELD } = { ...LOCUS_LIST_ITEMS_FIELD }
const LOCUS_LIST_RAW_ITEMS_FIELD = { ...LOCUS_LIST_ITEMS_BASE_FIELD, name: 'filter.rawItems', rows: 8, width: 7 }
const LOCUS_LIST_GREEN_ITEMS_FIELD = { ...LOCUS_LIST_ITEMS_BASE_FIELD, name: 'filter.rawItemsGreen', label: 'Green Genes', labelHelp: PA_LABEL_HELP, rows: 8, width: 3 }
const LOCUS_LIST_AMBER_ITEMS_FIELD = { ...LOCUS_LIST_ITEMS_BASE_FIELD, name: 'filter.rawItemsAmber', label: 'Amber Genes', labelHelp: PA_LABEL_HELP, rows: 8, width: 3 }
const LOCUS_LIST_RED_ITEMS_FIELD = { ...LOCUS_LIST_ITEMS_BASE_FIELD, name: 'filter.rawItemsRed', label: 'Red Genes', labelHelp: PA_LABEL_HELP, rows: 8, width: 3 }
...
const LocationFilter = React.memo(({ value, onChange }) => {
  ...
  const handleChange = (currFilterValue, filterField) => (_, newValue) => {
    ...
    if (currFilterValue.isPanelAppList) {
      result = { ...result, rawItems: toUniqueCsvString([result.rawItemsGreen, result.rawItemsAmber, result.rawItemsRed]) }
    }
