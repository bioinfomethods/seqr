...
do
  ...
  # Give the node some time to drain
  # If we do this too fast we will hit the pdb disruption error
  # ERROR: Cannot evict pod as it would violate the pod's disruption budget.
  # See: kubectl get pdb -n default
  # Note: If this is still happening you will need to wait and manually run the cordon, drain steps when the node is ready to be drained
  sleep 30;
done
